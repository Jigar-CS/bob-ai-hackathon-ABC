    ...
    assignments.append(
        {
            ...
            "weather_severity": (
                "moderate" if weather_delay > 1.5 else "minor" if weather_delay > 0 else "clear"
            ),
...
output = {
    ...
    "kpis": {
        ...
        "total_predicted_demurrage_usd": sum(
            a["predicted_demurrage_cost_usd"] for a in assignments
        ),
...
for a in assignments:
    report += (
        f"{a['vessel_id']}: {a['berth_id']} (Score: {a['ml_allocation_score']:.1f}, "
        f"Wait: {a['wait_hours']:.1f}h, Weather: {a['weather_delay_hours']:.1f}h)\n"
    )
...
print("\nFiles generated:")
print("  - ml_operations_plan.json      : Full JSON output with ML predictions")
print("  - ml_operations_report.txt     : Human-readable summary report")
print("  - ml_berth_assignments.csv     : Assignment details in CSV format")
print("  - ml_congestion_forecast.csv   : Congestion risk forecast data")
print(report)
