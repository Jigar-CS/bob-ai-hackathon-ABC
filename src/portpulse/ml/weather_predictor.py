...
class WeatherPredictor:
    """ML-based weather delay prediction system."""
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            if feature_cols_path.exists():
                ...
                WeatherPredictor._shared_feature_columns = feature_columns.get('weather_delay_features', [])
                self._feature_columns = WeatherPredictor._shared_feature_columns
            ...
            try:
                ...
                return {
                    ...
                    'factors': self._identify_factors(
                        ...
                        storm_probability, precipitation_mm
                    ),
                    'model_used': 'ml',
                }
            except Exception as err:
    ...
    def _classify_severity(self, wave_height_m: float, wind_speed_kt: float) -> str:
        ...
        if wave_height_m >= SEVERE_WAVE_M or wind_speed_kt >= SEVERE_WIND_KT:
            ...
            return "moderate"
        elif wave_height_m >= MINOR_WAVE_M or wind_speed_kt >= MINOR_WIND_KT:
            return "minor"
        elif wave_height_m >= 1.2 or wind_speed_kt >= 18.0:
            return "minor"
        return "none"
    ...
    def _calculate_confidence(
        ...
        # Average confidence
        base_confidence = 0.6 + 0.4 * np.mean([wave_conf, wind_conf, storm_conf])
...
def estimate_oceanic_conditions(
    lat: float, lon: float, eta: datetime
) -> tuple[float, float]:
...
def extract_weather_conditions(
    ...
    try:
        ...
        for i, t_str in enumerate(times):
            ...
            if (window_start <= t <= window_end) or (eta and abs(t.hour - eta.hour) <= 3 and not wave_values):
        ...
        if not wave_values or not wind_values or (max(wave_values or [0]) == 0 and max(wind_values or [0]) == 0):
...
def predict_route_weather_delays(
    ...
    # Get waypoints
    waypoints = calculate_route_waypoints(
        origin_lat, origin_lon, dest_lat, dest_lon, n_waypoints
    )
    ...
    for lat, lon in waypoints:
        ...
        max_wave, max_wind, avg_wave, avg_wind = extract_weather_conditions(
    ...
    # Aggregate delays (sum of waypoint delays, capped)
    if waypoint_delays:
        total_delay = min(24.0, sum(w['delay_hours'] for w in waypoint_delays))
        avg_confidence = np.mean([w.get('delay_hours', 0) for w in waypoint_delays]) / max(total_delay, 0.1)
    else:
        total_delay = 0.0
        avg_confidence = 0.5
        max_severity = "none"
...
def apply_ml_weather_delays(
    ...
    def process_vessel(vessel_row: Row) -> tuple[Row, dict[str, Any] | None]:
        ...
        try:
            target_lat = float(vessel.get("dest_lat") or port_lat)
            target_lon = float(vessel.get("dest_lon") or port_lon)
        except (TypeError, ValueError):
            target_lat, target_lon = port_lat, port_lon
        
        if has_origin:
            result = predict_route_weather_delays(
                o_lat, o_lon, target_lat, target_lon,
                eta, vessel_size, n_waypoints, timeout
            )
