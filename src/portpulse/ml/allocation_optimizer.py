...
class BerthAllocationOptimizer:
    ...
    def score_berth_vessel_pair(
        ...
        n_berths: int,
        n_vessels_queue: int,
        weather_severity: int = 0,  # noqa: ARG002
        hour_of_day: int = 12,  # noqa: ARG002
    ) -> dict[str, Any]:
        """
        ...
        score = self._rule_based_score(vessel, berth, wait_hours, capacity_fit, n_vessels_queue)
    ...
    def _rule_based_score(
        ...
        wait_hours: float,
        capacity_fit: float,
        n_vessels_queue: int,  # noqa: ARG002
    ) -> float:
    ...
    def _generate_recommendation(
        ...
        berth: BerthInfo,
        score: float,
        predicted_wait: float,  # noqa: ARG002
        capacity_fit: float,
        ...
        if capacity_fit < 1.2:
            reasons.append(f"tight fit ({capacity_fit:.1f}x capacity)")
        elif capacity_fit > 2.0:
            reasons.append(f"ample capacity ({capacity_fit:.1f}x)")
    ...
    def allocate_vessels(
        ...
        # Sort vessels by priority, then weather-adjusted arrival time so non-delayed
        # vessels fill empty berth slots
        vessel_infos.sort(
            key=lambda v: (
        ...
        for vessel in vessel_infos:
            ...
            if vessel.weather_delay_hours > 0 and "weather delay" not in reason_str:
            ...
            assignment = {
                ...
                "predicted_wait_hours": best_result["predicted_wait_hours"]
                if best_result
                else best_wait,
        ...
        logger.info(
            "ML allocation complete: %d assigned, %d unassigned", len(assigned), len(unassigned)
        )
    ...
    def _get_ml_predictions(
        ...
        berth: BerthInfo,
        wait_hours: float,
        n_berths: int,  # noqa: ARG002
        n_vessels: int,  # noqa: ARG002
    ) -> dict[str, Any]:
...
_optimizer: BerthAllocationOptimizer | None = None
